...
const { state, saveState } = useSingleFileAuthState(`./FanzBotVip.json`)
...
require('./fanz.js')
nocache('../fanz.js', module => console.log(color('[ UPDATE ]', 'green'), color(`'${module}'`, 'green'), 'Telah Di Perbarui'))
...
browser: ['FanzBotVip','Safari','1.0.0'],
auth: state,
version
})
...
console.log(color(figlet.textSync(`FanzBotVip`, {
font: 'Standard',
...
fanz = smsg(ap, kay, store)
require('./ap')(ap, fanz, chatUpdate, store)
} catch (err) {
console.log(err)}})
...
ap.serializeM = (fanz) => smsg(ap, fanz, store)
